...
# version, not the release. Keep both webui and cube-webui (registry name).
COMPONENTS='cube-egress|cube-egress-net|cube-master|cubemastercli|cube-api|cube-ops|cube-proxy|cube-webui|webui|cube-lifecycle-manager|cubelet|network-agent|cube-shim|cube-kernel|cube-guest|cube-node-init|cube-wait-node-prep|cube-pvm-host-bootstrap'
